...
function NavBar({page}) {
  ...
  return (
    <nav className="bg-stone-700 p-2 text-center flex flex-col gap-5">
      ...
      <Link to={`/admin/${adminReducer.uid}/manage-residents`} className={`text-md text-stone-100 p-2 hover:bg-stone-600 flex items-center justify-center gap-2 ${page === 2 && "bg-stone-600"}`}>
        {page === 2 ? <HiUsers className="h-5 w-5 text-gray-100" /> : <HiOutlineUsers className="h-5 w-5 text-gray-100" />}
        Manage Residents
      </Link>
      <Link to={`/admin/${adminReducer.uid}/manage-gym`} className={`text-md text-stone-100 p-2 hover:bg-stone-600 flex items-center justify-center gap-2 ${page === 3 && "bg-stone-600"}`}>
        {page === 3 ? <HiCalendarDays className="h-5 w-5 text-gray-100" /> : <HiOutlineCalendarDays className="h-5 w-5 text-gray-100" />}
        Manage Gymroom
      </Link>
      <Link to={`/admin/${adminReducer.uid}/manage-announcement`} className={`text-md text-stone-100 p-2 hover:bg-stone-600 flex items-center justify-center gap-2 ${page === 4 && "bg-stone-600"}`}>
        {page === 4 ? <HiMegaphone className="h-5 w-5 text-gray-100" /> : <HiOutlineMegaphone className="h-5 w-5 text-gray-100" />}
        Manage Announcements
      </Link>
      <Link to={`/admin/${adminReducer.uid}/manage-messages`} className={`text-md text-stone-100 p-2 hover:bg-stone-600 flex items-center justify-center gap-2 ${page === 5 && "bg-stone-600"}`}>
        {page === 5 ? <HiEnvelope className="h-5 w-5 text-gray-100" /> : <HiOutlineEnvelope className="h-5 w-5 text-gray-100" />}
        Manage Messages
      </Link>
      <Link to={`/admin/${adminReducer.uid}/change-password`} className={`text-md text-stone-100 p-2 hover:bg-stone-600 flex items-center justify-center gap-2 ${page === 6 && "bg-stone-600"}`}>
