...
function GymCalendar({ sinlgeGymScheduleDay, toggleRerender, setContextmenuInfo }) {
  ...
  return (
    <article className="text-center border border-stone-200 overflow-hidden">
      ...
      {allGymScheduleTimeslots.map((timeslot) => (
        <GymCalendarDay key={timeslot.timeslotId} dayId={sinlgeGymScheduleDay.dayId} singleGymScheduleTimeslot={timeslot} setContextmenuInfo={setContextmenuInfo} />
      ))}
    </article>
  );
}
