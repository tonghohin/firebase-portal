  ...
  return clickedAnnouncement.isInEditMode ? (
    <motion.form
      ...
      <nav className="flex justify-end">
        <HiXMark className="cursor-pointer h-5 w-5 rounded text-gray-600 hover:bg-gray-300 transition" onClick={() => setClickedAnnouncement({ ...clickedAnnouncement, isInEditMode: false })} />
      </nav>
      <input className="bg-gray-100 m-1 ml-0 border-2 w-full" type="text" name="subject" placeholder="Subject" onChange={handleChange} value={clickedAnnouncement.subject} required={true} autoComplete="false" />
    ...
    <article id={announcement.id} className="bg-gray-100 rounded p-2 grid grid-cols-2 cursor-pointer hover:bg-gray-200" onContextMenu={handleContextmenu}>
...
function Contextmenu({ setClickedAnnouncement, clickedAnnouncement }) {
  ...
  return (
    <button className="bg-white border border-gray-500 px-1 rounded fixed hover:bg-gray-300" style={{ left: clickedAnnouncement.coor.x, top: clickedAnnouncement.coor.y }} onClick={handleClick}>
      Edit
