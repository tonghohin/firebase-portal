    ...
    <section className="p-2 text-md bg-white mt-2 border-2 border-cyan-600 rounded">
      ...
      {messages.length !== 0 ? (
        messages.map((messageObj) => {
          return (
            <div key={messageObj.id} className="grid grid-cols-2 gap-1 mt-2">
              <article className="bg-slate-300 rounded p-2 grid grid-cols-2">
                <h1 className="font-bold">{messageObj.subject}</h1>
                <time className="text-xs text-right leading-5">{new Date(messageObj.createdAt._seconds * 1000).toDateString()}</time>
                <p className="text-gray-600 whitespace-pre-wrap">{messageObj.message}</p>
              </article>
              <article className="bg-slate-100 rounded p-2">
                {messageObj.reply ? (
                  <>
                    <time className="text-xs text-right block">Replied on {new Date(messageObj.updatedAt._seconds * 1000).toDateString()}</time>
