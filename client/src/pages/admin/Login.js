  ...
  return (
    <main className="col-span-full bg-main-bg bg-cover">
      <div className="backdrop-blur-sm backdrop-contrast-50 h-full w-full">
        ...
        <form className="text-center mt-40 bg-gray-300 border-2 rounded border-gray-300 w-96 h-72 m-auto flex flex-col items-center justify-around" onSubmit={handleSubmit}>
          ...
          <input type="password" name="password" className="rounded bg-gray-100 p-2 w-4/5" placeholder="Password" value={formData.password} onChange={handleChange} autoComplete="false" required={true}></input>
          {isInvalidEmailOrPassword && <p className="text-red-600">Invalid email/password</p>}
          <button className="bg-gray-500 text-white rounded p-1 w-20 hover:bg-gray-600">Login</button>
          <Link to="/admin/register" className="underline text-gray-600 hover:text-black">
            Register
          </Link>
          <Link to="/admin/forget-password" className="underline text-gray-600 hover:text-black">
            Forget password
          </Link>
        </form>
      </div>
